...
本项目基于原作者的prototype-html，基于个人工作习惯进行了功能优化，主要在原作者的基础上增加了以下内容：

1.全局组件说明，避免了在多个页面重复描述相同的全局组件。

2.prd内容优化：源项目在页面实现后，对于prd的内容比较简洁，因此我对skill进行了调整，要求尽可能完整的把功能进行描述。

3.可隐藏prd侧边栏：源项目prd内容不可隐藏，我增加了展开/隐藏按钮，方便预览web端产品的原有页面布局样式。
...
两侧之间通过**SVG 贝塞尔曲线**相连，连线会绑定到左侧核心功能区和右侧说明框，而不是写死坐标。页面滚动时连线会跟随两端元素实时重算；任一端滚出可视区时，连线自动隐藏。鼠标悬停在左侧核心功能区上，连线和右侧说明会同时高亮；悬停在右侧说明上，左侧对应区域也会高亮。这种"指哪打哪"的视觉关联，让原型和文档第一次真正融为一体。
...
### 第二步：先盘点功能清单
AI 会先区分**全局通用组件**和**页面核心功能区**。全局导航、搜索、消息提醒这类能力只说明一次；每个页面至少拆出 2 个核心功能区，避免把复杂页面压缩成一张笼统说明卡。

### 第三步：设计界面与说明映射
AI 会规划左侧界面区域与右侧说明文档的对应关系，优先选择当前页面最重要的 2-4 个核心功能区建立**连线标注**。这些连线不是装饰，而是帮助观者快速建立"界面区域 ↔ 功能含义"的认知映射。

### 第四步：生成完整 HTML 代码
AI 输出一个可直接运行的单文件 HTML，包含：
- 基于 Tailwind CSS 的现代 UI 样式
- Font Awesome 图标
- SVG 动态连线（绑定组件和说明框，滚动、显隐、窗口 resize 时自适应）
- 完整的交互逻辑（模态框、表格选中、hover 效果等）
- 覆盖组件与字段、操作逻辑、数据逻辑、边界结果的功能说明
